	...
	const unsigned int * tris, size_t tsize)
{
	...
	// Attributes let us tell openGL how the memory is laid out (used in shaders, position in array must be the same as in shaders)
	glEnableVertexAttribArray(0); // Position
	glEnableVertexAttribArray(1); // Color
	glEnableVertexAttribArray(2); // Normal
	glEnableVertexAttribArray(3); // TexCoord

	// index attributes, number of elements, type, normalized?, size of vertex, offset
